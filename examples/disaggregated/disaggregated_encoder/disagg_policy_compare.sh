...
# L3 connector selection (l3-clean): default = upstream example (pure storage);
# set EC_CONNECTOR=ECCacheConnector (+ optional EC_CACHE_MAX_BYTES) for bounded cache.
EC_CONNECTOR="${EC_CONNECTOR:-ECExampleConnector}"
...
start_disagg() {
  ...
  env VLLM_ENCODER_CACHE_POLICY="$policy" \
      ...
      vllm serve "$MODEL" \
        ...
        --ec-transfer-config "{\"ec_connector\":\"$EC_CONNECTOR\",\"ec_role\":\"ec_producer\",\"ec_connector_extra_config\":$EC_EXTRA}" \
        >"$enc_log" 2>&1 &
  PIDS+=($!)
  ...
  for i in "${!GPU_PD_ARR[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES="$pd_gpu" \
        vllm serve "$MODEL" \
          ...
          --ec-transfer-config "{\"ec_connector\":\"$EC_CONNECTOR\",\"ec_role\":\"ec_consumer\",\"ec_connector_extra_config\":$EC_EXTRA}" \
          >"$this_pd_log" 2>&1 &
    PIDS+=($!)
  ...
  ( cd "${GIT_ROOT}/examples/disaggregated/disaggregated_encoder" &&
    EC_STORE="$EC_STORE" SKIP_E="${SKIP_E:-0}" \
