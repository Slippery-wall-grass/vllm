...
# NOTE(l3-clean): encoder_cache_lambda is 余博's offline-policy module, absent on
# clean upstream. Only `cmd_solve` needs it → lazy-import inside cmd_solve so that
# `generate`/`measure`/`prewarm` work without the policy code.
...
def cmd_solve(args: argparse.Namespace) -> None:
    from vllm.v1.core.encoder_cache_lambda import (  # noqa: F401
        TypeStats,
        dual_value,
        solve_lambda_star,
    )
